...
输出视频使用 **fragmented MP4** 封装（`-movflags +frag_keyframe+empty_moov+default_base_moof`），
索引随每一片数据一起写入，因此**转换进行中就能用播放器打开输出文件**，不必等转换完成：

- 直接用 PotPlayer / VLC / MPC-HC 等打开输出路径即可；
- 可以拖动进度条到**已经编码完成的部分**，播放到最新写入位置会自动停止；
- 视频会随时间持续增长，重新拖动即可看到新的内容；
- 音轨在**转换完成后**混入（音频作为 ffmpeg 第二输入会引发编码器内存无界
  增长，8K 实测 3GB→10GB，故默认关闭；确需转换中出声可设环境变量
  `SHARP3D_LIVE_AUDIO=1` 强制启用，自行承担内存占用）；
- 转换完成或中途取消后，文件都会正常收尾、可完整播放（取消时保留已编码部分）。

> 说明：首片约在 1 秒素材（按帧率折算）编码完成后落盘，因此高分辨率/大量输出时
> 打开文件后可能需稍等片刻才出现内容。若播放器较老、不支持 fragmented MP4，
> 可等转换结束后再播放——最终文件与普通 MP4 用法完全一致。
