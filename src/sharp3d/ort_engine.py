...
def _interpolate_pos_embed(pos_embed: torch.Tensor,
                           old_grid: int, new_grid: int) -> torch.Tensor:
    """Interpolate ViT position embedding for a different grid size.

    Args:
        pos_embed: [1, 1 + old_grid*old_grid, dim] position embedding.
        old_grid: Original grid size (e.g. 24 for 384/16).
        new_grid: Target grid size (e.g. 20 for 320/16).

    Returns:
        Interpolated position embedding [1, 1 + new_grid*new_grid, dim].
    """
    import torch.nn.functional as F
    cls_token = pos_embed[:, :1, :]           # [1, 1, dim]
    patch_pos = pos_embed[:, 1:, :]           # [1, old_grid², dim]
    dim = patch_pos.shape[-1]
    # Reshape to 2D grid → interpolate → reshape back
    patch_pos = patch_pos.reshape(1, old_grid, old_grid, dim).permute(0, 3, 1, 2)
    patch_pos = F.interpolate(patch_pos.float(), size=(new_grid, new_grid),
                              mode='bicubic', align_corners=False)
    patch_pos = patch_pos.permute(0, 2, 3, 1).reshape(1, new_grid * new_grid, dim)
    return torch.cat([cls_token, patch_pos], dim=1).to(pos_embed.dtype)


def export_patch_encoder_320(predictor, onnx_path: Path, device: torch.device):
    """Export patch_encoder with 320×320 input (reduced resolution for speed mode).

    Interpolates the position embedding from 24×24 to 20×20 grid, exports
    the model, then restores the original position embedding.
    """
    import copy
    patch_encoder = predictor.monodepth_model.monodepth_predictor.encoder.patch_encoder

    # Deep copy to avoid modifying the original model
    encoder_copy = copy.deepcopy(patch_encoder)
    encoder_copy.eval().to(device)

    # Interpolate position embedding: 24×24 → 20×20
    old_grid = 24   # 384 / 16
    new_grid = 20   # 320 / 16
    encoder_copy.pos_embed.data = _interpolate_pos_embed(
        encoder_copy.pos_embed.data, old_grid, new_grid)

    dummy = torch.randn(35, 3, 320, 320, device=device, dtype=torch.float32)
    export_onnx_model(
        encoder_copy, onnx_path, device, dummy,
        output_names=["features", "intermediates", "add_1197", "add_1785", "add_2373"],
    )
    del encoder_copy


def create_ort_patch_encoder_320(
    predictor,
    device: torch.device,
    ...
    """Create an ORT patch_encoder with 320×320 input (speed mode)."""
    try:
        import onnxruntime  # noqa: F401
    except ImportError:
        return None

    onnx_path = onnx_dir / "patch_encoder_320.onnx"
    if not onnx_path.exists():
        try:
            export_patch_encoder_320(predictor, onnx_path, device)
        except Exception as e:
            logger.warning("320×320 ONNX export failed: %s", e)
            return None

    try:
        return ORTEncoder(onnx_path, trt_cache_dir, device,
                          label="patch_encoder",
                          grid_size=(20, 20),  # 320 / 16 = 20
                          int8_enable=int8_enable)
    except Exception as e:
        logger.warning("ORT 320 session creation failed: %s", e)
        return None
